	...
	Describe("Testing Parse method", func() {
		Context("with a basic tag block", func() {
			It("returns a valid value", func() {
				result, _ := Parse("\\s:Satellite_1,c:1553390539*62\\$GPRMC,001225,A,2832.1834,N,08101.0536,W,12,25,251211,1.2,E,A*03")
				rmc := result.(RMC)
		...
		Context("with a tag block with all tags", func() {
			It("returns a valid value", func() {
				result, _ := Parse("\\s:satellite,c:1564827317,r:1553390539,d:ara,g:bulk,n:13,t:helloworld*3F\\$GPRMC,001225,A,2832.1834,N,08101.0536,W,12,25,251211,1.2,E,A*03")
				rmc := result.(RMC)
				Expect(rmc.TagBlock.Valid).To(BeTrue())
				Expect(rmc.TagBlock.Time).To(Equal(NewInt64(1564827317)))
				...
				Expect(rmc.TagBlock.Source).To(Equal(NewString("satellite")))
		...
		Context("with a tag block with an empty tag", func() {
			It("returns a valid value", func() {
				result, _ := Parse("\\s:satellite,,c:1564827317,r:1553390539,d:ara,g:bulk,n:13,t:helloworld*13\\$GPRMC,001225,A,2832.1834,N,08101.0536,W,12,25,251211,1.2,E,A*03")
				rmc := result.(RMC)
		...
		Context("with a tag block with an invalid checksum", func() {
			It("returns a valid value", func() {
				result, _ := Parse("\\s:satellite,c:1564827317,r:1553390539,d:ara,g:bulk,n:13,t:helloworld*3A\\$GPRMC,001225,A,2832.1834,N,08101.0536,W,12,25,251211,1.2,E,A*03")
				rmc := result.(RMC)
				Expect(rmc.TagBlock.Valid).To(BeFalse())
		...
		Context("with a tag block without a checksum", func() {
			It("returns a valid value", func() {
				result, _ := Parse("\\s:satellite,c:156482731749\\$GPRMC,001225,A,2832.1834,N,08101.0536,W,12,25,251211,1.2,E,A*03")
				rmc := result.(RMC)
				Expect(rmc.TagBlock.Valid).To(BeFalse())
